...
function header() {
  return `<header class="site-header"><a class="wordmark" href="/" data-link><img src="/assets/mark-v1.svg" alt="" width="36" height="36"><span>Loudness Lens</span></a><nav aria-label="Main navigation"><a href="/demo" data-link>Demo</a><a href="/#how">How it works</a><a href="/privacy" data-link>Privacy</a><a class="download-small" href="/downloads/loudness-lens-chrome-1.0.0.zip" download>Download</a></nav></header>`;
}
...
function shell(content: string, demo = false) {
  return `${demo ? `<aside class="demo-banner" aria-label="Demo mode"><strong>Demo — sample data, nothing is saved</strong><span><button id="reset-demo" type="button">Reset demo</button><a id="start-real" href="/downloads/loudness-lens-chrome-1.0.0.zip" download>Start for real</a></span></aside>` : ''}${header()}${content}${footer()}`;
}
...
function home() {
  return shell(`<main id="main">
    ...
    <section class="limits" aria-labelledby="limits-title"><div><p class="kicker">Clear boundaries</p><h2 id="limits-title">What the guard does not do</h2></div><ul><li>It starts only when you turn it on for a tab.</li><li>It does not capture background tabs by itself.</li><li>Some tabs cannot provide audio to Chrome extensions.</li><li>It cannot guarantee a safe listening level.</li></ul></section>
    <section class="install"><p class="kicker">Chrome extension · v1</p><h2>Put the guard beside your address bar</h2><p>Download the package, open Chrome extensions, and load the unpacked folder.</p><a class="button primary" href="/downloads/loudness-lens-chrome-1.0.0.zip" download>Download Loudness Lens</a></section>
  </main>`);
}
